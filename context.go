...
import (
	...
	"github.com/vannnnish/easyweb/binding"
...
func (c *Context) Success(obj interface{}) error {
	...
	data["msg"] = ""
	// 在这里进行一点修改为了适应redis缓存
	if hasCache := c.StoreGetInt("has_cache"); hasCache == 1 {
		j, _ := json.Marshal(data)
		c.StoreSet("cache", string(j))
	}
	return c.JSON(200, data)
}

func (c *Context) EncryptSuccess(key []byte, obj interface{}) error {
	data := make(map[string]interface{})
	data["data"] = obj
	data["code"] = defaultSuccessCode
	data["msg"] = ""
	...
	jsonBytes, err = aes.AesEncrypt(key, jsonBytes)
	if err != nil {
		return err
	}
	// 在这里进行一点修改为了适应redis缓存
	if hasCache := c.StoreGetInt("has_cache"); hasCache == 1 {
		c.StoreSet("cache", string(jsonBytes))
	}
	return c.Data(200, "application/json; charset=utf-8", jsonBytes)
}

func (c *Context) SuccessWithCache(obj interface{}) error {
	data := make(map[string]interface{})
	data["data"] = obj
	data["code"] = defaultSuccessCode
	data["msg"] = ""
...
func (c *Context) SuccessAndMsg(obj interface{}, msg string) error {
	data := make(map[string]interface{})
	data["data"] = obj
	data["code"] = defaultSuccessCode
	data["msg"] = msg
...
func (c *Context) AesSuccess(obj interface{}) error {
	data := make(map[string]interface{})
	key := c.StoreGetString("key")
	if key == "" {
		data["data"] = obj
	} else {
		jsonData, _ := json.Marshal(obj)
		encryptData, _ := aes.AesEncrypt([]byte(key), jsonData)
		data["data"] = string(encryptData)
	}
	data["code"] = defaultSuccessCode
	data["msg"] = ""
	return c.JSON(200, data)
...
func (c *Context) AesSuccessWithMsg(obj interface{}, msg string) error {
	data := make(map[string]interface{})
	key := c.StoreGetString("key")
	if key == "" {
		data["data"] = obj
